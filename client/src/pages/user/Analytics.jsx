...
export default function Analytics() {
  ...
  function loadLogs(page) {
    api.getMyVisitorLogs(page, 50).then(data => {
      setLogs(data.logs);
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1>Analytics</h1><p>Visitor statistics for your landing pages.</p></div>
      </div>
      ...
      <div className="stats-grid" style={{ marginBottom: 20 }}>
        ...
        <div className="stat-card stat-green">
          ...
          <div><div className="stat-value">{stats?.uniqueIps || 0}</div><div className="stat-label">Unique Visitors</div></div>
        </div>
      ...
      <div className="section-card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        {logs.length === 0 ? (
          <div className="empty-state"><p>No visitors yet. Share your links to start getting traffic.</p></div>
        ) : (
          <>
            <div style={{ overflowX: 'auto' }}>
              <table className="data-table">
                <thead><tr><th>IP</th><th>Browser</th><th>Country</th><th>City</th><th>ISP</th><th>Path</th><th>Time</th></tr></thead>
                <tbody>
                  {logs.map(v => (
                    <tr key={v.id}>
